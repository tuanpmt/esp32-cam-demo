...
static void httpdParseHeader(char *h, HttpdConnData *conn) {
    ...
    if (strncmp(h, "SUBSCRIBE ", 10)==0) {
        ...
        while (h[i]==' ') i++;
        conn->hostName=&h[i];
    } else if (strncmp(h, "ANNOUNCE ", 9)==0) {
        conn->requestType = HTTPD_METHOD_ANNOUNCE;
        firstLine=1;
    } else if (strncmp(h, "RECORD ", 7)==0) {
        conn->requestType = HTTPD_METHOD_RECORD;
        firstLine=1;
    } else if (strncmp(h, "SETUP ", 6)==0) {
        conn->requestType = HTTPD_METHOD_SETUP;
        firstLine=1;
    } else if (strncmp(h, "PAUSE ", 6)==0) {
        conn->requestType = HTTPD_METHOD_PAUSE;
        firstLine=1;
    } else if (strncmp(h, "FLUSH ", 6)==0) {
        conn->requestType = HTTPD_METHOD_FLUSH;
        firstLine=1;
    } else if (strncmp(h, "TEARDOWN ", 9)==0) {
        conn->requestType = HTTPD_METHOD_TEARDOWN;
        firstLine=1;
    } else if (strncmp(h, "OPTIONS ", 8)==0) {
        conn->requestType = HTTPD_METHOD_OPTIONS;
        firstLine=1;
    } else if (strncmp(h, "GET_PARAMETER ", 13)==0) {
        conn->requestType = HTTPD_METHOD_GET_PARAMETER;
        firstLine=1;
    } else if (strncmp(h, "SET_PARAMETER ", 13)==0) {
        conn->requestType = HTTPD_METHOD_SET_PARAMETER;
        firstLine=1;
    }
    ...
    if (firstLine) {
        ...
        *e=0; //terminate url part
        ...
        if (strcasecmp(e, "HTTP/1.1")==0 || conn->requestType >= 5) conn->priv->flags|=HFL_HTTP11|HFL_CHUNKED;
...
void httpdAddRouter(HttpdBuiltInUrl router[], void *context)
{
    int i = 0;
    if(is_init == 0)
        httpdInit(80); //default port
    HttpdBuiltInUrlTree *tree = builtInUrlTrees;
    while(router[i].url != NULL) {
        while (tree->next != NULL) {
            tree = tree->next;
        }
        tree->next = calloc(1, sizeof(HttpdBuiltInUrlTree));
        ((HttpdBuiltInUrlTree*)tree->next)->ptr = &router[i];
